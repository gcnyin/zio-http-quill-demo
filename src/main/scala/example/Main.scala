...
object Main extends ZIOAppDefault {
    ...
    Http.collectZIO[Request] {
      ...
      case req@Method.POST -> Root / "user" =>
        val response = for {
          ...
          index <- userRepository.createUser(username = request.username, password = request.password).mapError(e => ErrorMsg("INTERNAL_ERROR", e.getMessage))
          _ = println(index)
  ...
  override def run: ZIO[Any with ZIOAppArgs with Scope, Any, Any] = {
    val quillLayer: ZLayer[DataSource, Nothing, Quill.Postgres[CompositeNamingStrategy]] =
      Quill.Postgres.fromNamingStrategy(CompositeNamingStrategy2(PostgresEscape, SnakeCase))
